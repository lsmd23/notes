...
- 调整参数的步骤：
	...
	- **反向传播**：计算各个参数的调整幅度
		...
		- 求得的梯度值在每个向量上的分量，表示损失函数的变化在该分量对应的参数$\theta_i$上的敏感程度
	- 调整参数：根据计算的梯度，按一定的学习率（也即梯度下降的步长），进行参数的更新，之后反复迭代即可
## 训练策略
- 问题：数学证明发现，对深度学习的损失函数，其局部极值的个数是指数量级的。因此，希望能有更多的手段，逃离局部极值，得到泛化性更好的局部极值点
- 方案：
	- 随机梯度下降(SGD)：随机选取一部分的维度计算梯度
		- 说明：也即每次并不是更新所有的参数，而是只选取一部分的参数进行更新，之后再选取另一批参数进行更新
		- 优势：计算快，且**逃离鞍点**
	- 动量随机梯度下降：借鉴物理中的冲量与动量概念，引入一个冲量值
		- 数学公式：$$\Delta = \beta\Delta+(1-\beta)\frac{\partial}{\partial \theta_{ij}^{(l)}}J(\theta,b)$$
		- 优势：可以逃离比较浅的局部极值
		- 实际应用：ADM优化
	- 学习率（衰减）策略：
	...
	- 参数丢弃：
		...
		- 优势：
			...
			- 类似于随机森林的思想
	- 权重初始化：
		- 对参数的初始化会影响学习的成果，合适的初始化可能会大幅提升学习的效率
- 总结：
	- 实际过程中，数据集、学习率、学习的程度与结果的相关性：![[Pasted image 20250421154724.png]]
# 卷积神经网络(CNN)
...
- 卷积神经网络(CNN)：
	- 卷积运算：
		...
		- 交叉相关函数：处理复变函数的卷积运算$$(f\star g)(n)=\sum_{m=-\infty}^\infty \overline{f(m)}g(m+n)$$也即用酉空间中定义的内积代替普通的乘法内积
