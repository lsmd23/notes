...
#### SLR(1)分析
- 基于LR(0)自动机实现，根据下一个输入符号是否属于带归约的非终结符的Follow集合，决定是否进行归约
- **SLR(1)分析表**
	- 构造：对文法$G[S]$，其拓广文法为$G'[S']$，以及其LR(0)项目集规范族$C=\{I_0,I_1,\cdots,I_n\}$，令状态$I_k$对应的栈顶状态为$k$；并令含有项目：$S'\rightarrow .S$的状态为$I_0$，也即初态，则SLR(1)分析表的构造步骤如下：
		1. 若$\text{GO}(I_i,A)=I_j$，其中$A$为非终结符，则令$\text{GOTO}[i,A]=j$
		2. 若项目$A\rightarrow \alpha . a \beta \in I_i$，其中$a$为终结符，且$\text{GO}(I_i,a)=I_j$，则令$\text{ACTION}[i,a]$为“移进j”，简记为“sj”
		3. 若项目$A\rightarrow \alpha . \in I_i$，其中$A\neq S'$，则对每个$b\in \text{Follow}(A)$，令$\text{ACTION}[i,b]$为“归约$A→α$”，简记为“rj”（假定产生式$A\rightarrow \alpha$为文法中的第$j$号产生式）
		4. 若项目$S'\rightarrow S. \in I_i$，则令$\text{ACTION}[i,\#]$为“接受”，简记为“acc”
		5. 对其他表项，令其为错误标记
	- 例：![[Pasted image 20251110204454.png]]
	- 说明：
		- ACTION表：告诉分析引擎在栈顶状态为$i$且当前输入符号为$a$时，应该执行何种操作
			- “sj”：移进操作，将输入符号$a$移入栈中，并将状态$j$压入栈中
			- “rj”：归约操作，使用产生式$A\rightarrow \alpha$对栈顶进行归约
			- “acc”：接受操作，分析成功
			- 错误标记：表示分析出错
		-  GOTO表：$\text{GOTO}[i,A]=j$表示在栈顶状态为$i$且依产生式$A\rightarrow \beta$归约后，应该将状态$j$压入栈中（同时弹出$|\beta|$个状态）
- **SLR(1)文法**：
	- 定义：若按照上述方法构造的SLR(1)分析表中不存在冲突，则称文法$G$为SLR(1)文法
	- 性质：在其LR(0)自动机中，每个状态都有：
		- 对任何项目$A\rightarrow u.av$，其中$a$为终结符，不存在项目$B\rightarrow w.$，使得$a\in \text{Follow}(B)$
		- 对该状态的任何两个项目$A\rightarrow u.$和$B\rightarrow v.$，满足$\text{Follow}(A)\cap \text{Follow}(B)=\emptyset$
- SLR(1)分析算法：
	- 算法描述：置 ip 指向输入串 w 的首符号，置初始栈顶状态为 0 ，令 i 为栈顶状态，a 是 ip 指向的符号，重复如下步骤：
		```text
		if (ACTION[i,a] = "sj") {
			/* 移进操作 */
		    push(a); /* 将输入符号 a 入栈 */
		...
		else if (ACTION[i,a] = "rk") {
		    /* 归约操作 */
		    A → β = the k-th production;
		    pop(|β|); /* 弹出 |β| 个栈元素 */
		    push(A); /* 将非终结符 A 入栈 */
		    t = top(); /* 取当前栈顶状态 */
		    push(GOTO[t,A]); /* 将 GOTO[t,A] 入栈 */
		}
		else if (ACTION[i,a] = "acc") {
		    /* 接受操作 */
		...
		```
	- 例：![[Pasted image 20251110205846.png]]
- 问题：只能解决一部分移进-归约冲突
	- 只考虑到非终结符的Follow集合，未考虑非终结符的Follow集合中的符号是否也是句柄的Follow符号
#### LR(1)分析
- **LR(1)项目**：
	- 在LR(0)项目的基础上增加了一个终结符，被称为**向前搜索符**，表示产生式右端完整匹配后允许在余留符号串中的下一个终结符
	- 形式：$A\rightarrow \alpha . \beta,a$，其中$A\rightarrow \alpha .\beta$是LR(0)项目，$a\in V_T\cup \{\#\}$为向前搜索符
	- 含义：
		- 对形如$A\rightarrow \alpha . ,a$的项目，表示当下一个输入符号为$a$时，才可以使用产生式$A\rightarrow \alpha$进行归约
		- 对其他形式项目，$a$传递向前搜索符的相关信息
	- 合并向前搜索符：
		- 对两个LR(1)项目$A\rightarrow \alpha . \beta,a$和$A\rightarrow \alpha . \beta,b$，可以合并为一个项目$A\rightarrow \alpha . \beta,\{a,b\}$
		- 合并后，向前搜索符变为一个集合
- **LR(1)自动机**：
	- 类似于LR(0)自动机，也可以构造对应的LR(1)自动机
	- 构造：
		- 状态：LR(1)项目集的闭包
			- 计算算法：![[Pasted image 20251110210715.png]]
		- 初态：设$G'[S']$是文法$G=(V_T,V_N,S,P)$的拓广文法，则初态为项目集闭包：$I_0=\text{CLOSURE}(\{S'\rightarrow .S,\#\})$
			- 例：![[Pasted image 20251110210737.png]]
		- 状态转移函数：$\text{GO}(I,X)=\text{CLOSURE}(J)$，其中，$I$为自动机的状态（也即项目集的闭包），$X$为文法符号，$J=\{[A\rightarrow \alpha X . \beta,a ]| [A\rightarrow \alpha . X \beta,a] \in I\}$
		- 项目集规范族：LR(1)自动机的所有状态的集合，可由如下算法计算![[Pasted image 20251110210937.png]]
	- 例：![[Pasted image 20251110211020.png]]![[Pasted image 20251110211112.png]]![[Pasted image 20251110211120.png]]
- **LR(1)分析表**：
	- 构造：假定$C=\{I_0,I_1,\cdots,I_n\}$为文法$G$的LR(1)项目集规范族，令状态$I_k$对应的LR(1)分析表的栈顶状态为$k$；令含有项目$[S'→.S ,\#]$的状态为$I_0$，因此0为初态，按如下算法构造LR(1)分析表：
		1. 若$\text{GO}(I_i,A)=I_j$，其中$A$为非终结符，且项目$[A\rightarrow \alpha .a\beta,b]\in I_i$，则令$\text{ACTION}[i,a]$为“移进j”，简记为“sj”
		2. 若项目$[A\rightarrow \alpha . ,a]\in I_i$，其中$A\neq S'$，则令$\text{ACTION}[i,a]$为“归约$A→α$”，简记为“rj”（假定产生式$A\rightarrow \alpha$为文法中的第$j$号产生式）
		3. 若项目$[S'\rightarrow S. ,\#]\in I_i$，则令$\text{ACTION}[i,\#]$为“接受”，简记为“acc”
		4. 若$\text{GO}(I_i,A)=I_j$，其中$A$为非终结符，则令$\text{GOTO}[i,A]=j$
		5. 对其他表项，令其为错误标记
	- 例：![[Pasted image 20251110211635.png]]![[Pasted image 20251110211650.png]]
- LR(1)文法：
	- 定义：若按照上述方法构造的LR(1)分析表中不存在冲突，则称文法$G$为LR(1)文法
	- 性质：在其LR(1)自动机中，每个状态都有：
		- 对任何项目$A\rightarrow u.av,b$，其中$a$为终结符，不存在项目$B\rightarrow w.,a$，使得$b$为向前搜索符
		- 对该状态的任何两个项目$A\rightarrow u.,a$和$B\rightarrow v.,b$，满足$a\neq b$
- *推广：可以进一步推广到LR(k)分析，但由于状态数目会指数增长，实际应用中较少使用*
#### LALR(1)分析
- 背景：
	- LR(1)分析虽然功能强大，但复杂度较高，状态数目较多
	- SLR(1)分析状态数目较少，但功能较弱
	- 因此，可以合并LR(1)自动机中的某些状态，从而减少状态数目
- **同芯状态**：
	- 定义：在LR(1)自动机中，若两个状态的**内核项相同**，则称这两个状态为同芯状态
		- 例：![[Pasted image 20251111132548.png]]![[Pasted image 20251111132557.png]]![[Pasted image 20251111132606.png]]
	- 同芯状态合并：将同芯状态合并为一个状态，合并后，项目的向前搜索符为原来**所有项目向前搜索符的并集**
		- 例：![[Pasted image 20251111132708.png]]
- **LALR(1)文法**：若将文法$G$的LR(1)自动机中所有同芯状态合并后，得到的新状态**无归约-归约冲突**，则称文法$G$为LALR(1)文法（合并一定不会产生移进-归约冲突）
- **LALR(1)自动机**构造：
	1. 暴力构造法：
		- 先构造文法$G$的LR(1)自动机
		- 然后将同芯状态合并，得到LALR(1)自动机 
		- 其转移函数为被合并的状态的转移函数的并集
		- 例：对上述的自动机，合并后为：![[Pasted image 20251111133052.png]]
	2. 逐步合并法：
		- 状态和转移函数的构造与LR(1)自动机类似，但在产生新状态后，立即检查是否与已有状态同芯，若是，则进行合并
- **LALR(1)分析表**构造：
	- 类似于LR(1)分析表的构造，但使用LALR(1)自动机
	- 例：![[Pasted image 20251111133202.png]]
- 特点：
	- 状态数目与LR(0)自动机相同
	- 功能性强于SLR(1)分析，弱于LR(1)分析
### 二义文法的LR分析
- 二义文法一定不是LR文法，但加入适当的优先级和结合性规则后，可以进行LR分析
	- 例：对如下表示的拓广文法![[Pasted image 20251111133625.png]]
		- 构造LR(0)自动机：![[Pasted image 20251111133725.png]]
		- 由于符号$+,*$都是非终结符$E$的Follow集合中的符号，因此存在移进-归约冲突。但若规定，$*$的优先级高于$+$，且两者均为左结合，则可以解决冲突
		- 对状态$I_9$，当输入符号为$+$时，进行归约操作；当输入符号为$*$时，进行移进操作；对状态$I_10$，当输入符号为$+$或$*$时，均进行归约操作
		- 进一步构造分析表如下：![[Pasted image 20251111133913.png]]
### LR分析中的错误处理
- 当栈顶状态为$i$且当前输入符号为$a$时，若$\text{ACTION}[i,a]$为错误标记，则报告错误
- 可以对表项做适度的进一步标记，以提供更有意义的错误信息，进一步选择对应的恢复策略
- 例：![[Pasted image 20251111134025.png]]
## ==分析文法间的关系==
- 示意图：![[Pasted image 20251111134049.png]]
---
[[L4 语法制导的翻译]]