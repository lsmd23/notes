...
- 马尔可夫奖励过程（Markov Reward Process, MRP）：
	...
	- 贝尔曼方程（Bellman Equation）：对状态价值函数，将其分解为动态规划的形式$$v(s) = \mathbb{E}[R_{t+1} + \gamma v(S_{t+1}) | S_t = s]$$ ^769904
		...
		- 因此，有：$$v(s)=\mathcal{R}_s+\gamma \sum_{s'\in \mathcal{S}}\mathcal{P}_{ss'}v(s') $$写成矩阵形式也即：$$\pmb v = \pmb{\mathcal{R}}+\gamma\pmb{\mathcal{Pv}}$$其中$\pmb v$为每个状态的价值向量：$$\begin{bmatrix} v(1) \\ \vdots \\ v(n) \end{bmatrix} = \begin{bmatrix} \mathcal{R}_1 \\ \vdots \\ \mathcal{R}_n \end{bmatrix} + \gamma \begin{bmatrix} \mathcal{P}_{11} & \cdots & \mathcal{P}_{1n} \\ \vdots & \ddots & \vdots \\ \mathcal{P}_{n1} & \cdots & \mathcal{P}_{nn} \end{bmatrix} \begin{bmatrix} v(1) \\ \vdots \\ v(n) \end{bmatrix}$$
## 马尔可夫决策过程
- 马尔可夫决策过程（Markov Decision Process, MDP）：
	- 定义：马尔可夫决策过程是一个元组$<\mathcal{S}, \mathcal{A}, \mathcal{P}, \mathcal{R}, \gamma>$，其中
		...
		- $\mathcal{R}$：奖励函数，$\mathcal{R}_s^a=\mathbb{E}[R_{t+1}|S_t=s,A_t=a]$，表示在状态$s$下采取动作$a$后，下一步获得的期望奖励（确定动作，但状态仍然是随机的）
		- $\gamma$：折扣因子
...
- 最优价值函数（Optimal Value Function）：
	- 定义：$$v_*(s) = \max_\pi v_\pi(s)$$表示在状态$s$下，智能体按策略$\pi$执行动作所能获得的最大累积奖励的期望$$q_*(s,a) = \max_\pi q_\pi(s,a)$$表示在状态$s$下，智能体采取动作$a$后，按策略$\pi$执行动作所能获得的最大累积奖励的期望
		- 在MDP中，最优价值函数可以确定对这个问题的一个最优策略
	- 例：![[Pasted image 20251208152931.png]]
- 最优策略（Optimal Policy）：
	- 定义：若策略$\pi$在所有状态下的价值都不低于其他策略$\pi'$，也即$v_\pi(s) \geq v_{\pi'}(s),\forall s \in \mathcal{S}$，则称$\pi \geq \pi'$，并称$\pi$为最优策略
	- 存在性定理：对MDP过程，一定存在最优策略$\pi_*$，使得$\pi_* \geq \pi, \forall \pi$，且所有的最优策略都能得到最优价值函数：$v_{\pi_*}(s) = v_*(s), q_{\pi_*}(s,a) = q_*(s,a), \forall s \in \mathcal{S}, a \in \mathcal{A}$
	- 求解过程：最大化最优动作价值函数
		- 对每个状态$s\in \mathcal{S}$，选择使得$q_*(s,a)$最大的动作$a$作为最优策略$\pi_*$在状态$s$下的动作选择，即$$\pi_*(a|s)=\begin{cases}1,&\text{if }a=\text{argmax}_a q_*(s,a)\\0,&\text{otherwise}\end{cases}$$
		- 最优策略是一个确定性策略（给定状态，动作确定）
- 贝尔曼最优方程（Bellman Optimality Equation）：
	- 根据上述的[[L6 强化学习(A)#^e54a68|贝尔曼方程]]的四个等价形式，可以得到四个贝尔曼最优方程：
		1. $$v_*(s)= \max_a q_*(s,a)$$
		2. $$q_*(s,a) = \mathcal{R}_s^a + \gamma \sum_{s' \in \mathcal{S}} \mathcal{P}_{ss'}^a v_*(s')$$
		3. $$v_*(s) = \max_a \left[ \mathcal{R}_s^a + \gamma \sum_{s' \in \mathcal{S}} \mathcal{P}_{ss'}^a v_*(s') \right]$$
		4. $$q_*(s,a) = \mathcal{R}_s^a + \gamma \sum_{s' \in \mathcal{S}} \mathcal{P}_{ss'}^a \max_{a'} q_*(s',a')$$
	- 最优价值函数可以由贝尔曼最优方程递归的进行求解，虽然其不是一个线性方程组，但可以通过迭代方法进行近似求解
	- 定理：对任意的MDP，所有状态$s\in \mathcal{S}$，有$v_*(s) = \max_a q_*(s,a)$，证明如下：
		- 由定义：$$v_*(s) = \sum_{a\in \mathcal{A}} \pi_*(a|s) q_*(s,a)\leq \sum_{a\in \mathcal{A}} \pi_*(a|s) \cdot (\max_a q_*(s,a)) = \max_a q_*(s,a)$$
		- 如果取得不等号，则一定可以构造一个确定性的策略，使得在状态$s$下选择使得$q_*(s,a)$最大的动作$a$，从而得到$v_*(s) = \max_a q_*(s,a)$，与$v_*(s)$的定义矛盾
- 强化学习的分类：根据贝尔曼最优方程的选取和最优化过程，强化学习建立了多种模型
	- 价值学习模型：例如[[L6 学习(C)#基于价值的学习|人工智能导论——基于价值的学习]]
