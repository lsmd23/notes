...
- 集中不等式（Concentration Inequalities）：
	...
	- Hoeffding不等式（Hoeffding's Inequality）：
		- 定理：设$X_1,X_2,...,X_n$为独立随机变量，且每个变量均有界，即$X_i$的取值范围为$[a_i,b_i]$，记$V_i=X_i-\mathbb{E}[X_i]$，则对于任意$\epsilon > 0$，有$$P\left(\sum_{i=1}^n V_i \geq \epsilon\right) \leq \exp\left(-\frac{2\epsilon^2}{\sum_{i=1}^n (b_i - a_i)^2}\right)$$
...
- 定义：
	...
	- 计算：只需证明，对某个正整数$d$，存在某组$d$个数据点可以被假设空间$\mathcal{H}$打散，而对任意$d+1$个数据点，假设空间$\mathcal{H}$都不能打散，则有$\text{VCdim}(\mathcal{H}) = d$
