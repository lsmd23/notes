...
1. 分支跳转控制冒险和返回指令控制冒险同时发生（在跳转的目的指令流中有一个`ret`指令）
	...
	- 结合后的控制逻辑为：![[Pasted image 20251208212707.png]]
2. 加载/使用数据冒险和返回指令控制冒险同时发生（如前一条指令的`popq %rsp`这样从内存读取到栈顶地址的指令，紧跟的`ret`指令依赖该地址进行返回）
	- 如果其是错误跳转的指令流，理应忽略`mrmovq`指令对数据的影响，只关注跳转处理的指令影响
	- 因此，将返回指令的优先级提高，在遇到`ret`指令时暂停取指阶段即可
	- 结合后的控制逻辑为：![[Pasted image 20251208213023.png]]
