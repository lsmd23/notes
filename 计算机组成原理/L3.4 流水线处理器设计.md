...
- 综合实现：
	- 结合加载/使用冒险、分支跳转冒险和返回指令冒险的控制逻辑如下：![[Pasted image 20251208211325.png]]
	- 具体实现：
		```python
		# set F stage stall
		bool F_stall = 
			...
			# stalling when ret passes through pipeline
		...
		``` 
### 处理特殊情况
- 虽然单独解决了数据冒险和控制冒险的问题，但在实际的流水线处理器中，当二者同时出现时，由于解决方式可能会有冲突，因而需要对一些特殊情况进行处理
1. 分支跳转控制冒险和返回指令控制冒险同时发生（在跳转的目的指令流中有一个`ret`指令）
	- 如果其是错误跳转的指令流，理应忽略`ret`指令对控制的影响，只关注跳转处理的指令影响
	- 因此，将跳转指令的优先级提高，在遇到`ret`指令时暂停取指阶段即可
	- 二者结合后的控制逻辑为：![[Pasted image 20251208212707.png]]
2. 加载/使用数据冒险和返回指令控制冒险同时发生（如前一条指令的`popq %rsp`这样从内存读取到栈顶地址的指令，紧跟的`ret`指令依赖该地址进行返回）
	- 对加载/使用数据冒险，要求在执行阶段插入气泡等待数据，并同时对取指和译码阶段进行暂停
	- 对返回指令控制冒险，要求在译码阶段插入气泡，并暂停取指阶段
	- 应当优先让加载/使用数据冒险处理，因为其会影响数据的正确性，而返回指令的控制冒险只影响指令流的正确性
	- 二者结合后的控制逻辑为：![[Pasted image 20251208214417.png]]
- 实现：修正后的标志值如下：
```python
# set Decode bubble
bool D_bubble = 
...
# 处理器的性能分析
- 流水线处理器的性能指标：
	- 时钟频率（Clock rate）：单位时间内时钟周期的数量，通常以MHz或GHz为单位
	- 指令执行率（CPI, Cycles Per Instruction）：每条指令平均需要的时钟周期数
		- 对于理想的处理器，CPI=1（也即在每个时钟周期都处理一条指令）
		- 当存在冒险时，CPI>1（由于需要插入气泡或暂停流水线）
	- CPI的计算：
		- 记$C$为时钟周期数，$I$为指令数，则CPI=$C/I$
		- 记$B$为气泡数，则CPI=$1 + B/I$
		- 经验统计计算三种来源的气泡数：
			- $LP$：加载/使用冒险引入的气泡数
				- 加载指令的统计期望占比约0.25
				- 产生加载/使用冒险的概率约0.20
				- 每次加载/使用冒险引入1个气泡
				- 则$LP = 0.25 * 0.20 * 1 = 0.05$
			- $MP$：分支跳转冒险引入的气泡数
				- 分支跳转指令的统计期望占比约0.20
				- 预测错误的概率约0.40
				- 每次分支跳转冒险引入2个气泡
				- 则$MP = 0.20 * 0.40 * 2 = 0.16$
			- $RP$：返回指令冒险引入的气泡数
				- 返回指令的统计期望占比约0.02
				- 每次返回指令冒险引入3个气泡
				- 则$RP = 0.02 * 3 = 0.06$
			- 则总气泡数$B = LP + MP + RP = 0.05 + 0.16 + 0.06 = 0.27$，也即CPI=$1 + 0.27 = 1.27$
- 现代处理器的优化：
	- 分支跳转优化：通过优化使得预测正确的概率更高
		- 分支目标缓冲器（BTB, Branch Target Buffer）：存储分支跳转的历史信息，通过简单的自动机来预测跳转的方向
	- 现代处理器的并行：
		- 现代处理器将指令控制单元和执行单元进行分离，使得指令可以并行执行
			- 控制单元：![[Pasted image 20251215101613.png]]
				- 从指令缓存取指
				- 将指令翻译为操作（Operations）
				- 将寄存器引用转换为标签
			- 执行单元：![[Pasted image 20251215101703.png]]
				- 多个执行单元并行工作
				- 每个执行单元可以有自己的流水线，从而实现更高的并行度
		- 各个指令执行所需的周期数：
			- 加载/存储指令：4个周期
			- 整数乘法：3个周期
			- 整数除法：3-30个周期
